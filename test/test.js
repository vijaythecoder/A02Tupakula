...
QUnit.test('Testing calculateArea function with several sets of inputs', function (assert) {
    assert.equal(App.calculatePrice('02/21/2018', '02/28/2018', 2, true), 1680, 'Tested with two dates');
    assert.throws(function () { App.calculatePrice('', '', null, true); }, /Start Date is required/, 'Passing in empty start date raises an Error');
    assert.throws(function () { App.calculatePrice('02/21/2018', '', null, true); }, /End Date is required/, 'Passing in empty end date raises an Error');
    assert.throws(function () { App.calculatePrice(0,0,0, true); }, /Start date and End date cannot be equal/, 'Passing in null correctly raises an Error');
});
